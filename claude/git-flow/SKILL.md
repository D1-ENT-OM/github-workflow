---
name: git-flow
description: Git/GitHub collaboration workflow for AI-assisted development in Claude Code. Use when the user asks to start work, pull/update, create or switch branches, commit, push, open or update a PR, check merge readiness, merge, verify deployment, or says Korean phrases like "작업 시작", "다 했어", "올려줘", "PR 올려줘", "반영해줘", or "머지해줘". Helps identify parallel-work risk, prepare understandable PRs, and avoid unsafe main/production changes.
---
...
Use this skill in Claude Code with `/git-flow` or when the user's request clearly matches Git/GitHub collaboration. The goal is not code-quality review; it is to keep branch, commit, PR, merge, and deployment handoffs clear enough that non-expert users can collaborate safely with AI agents.

Prefer the repository's own rules over this generic workflow. Before acting, read relevant local guidance such as `CLAUDE.md`, `AGENTS.md`, `CONTRIBUTING.md`, `README.md`, `.github/pull_request_template.md`, and docs under `docs/` when they exist.
